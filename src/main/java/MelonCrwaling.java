...
public class MelonCrwaling {
    ...
    public void serchTimeInfo(){
        ...
        Elements times = body.select("span.yyyymmdd");
        Elements times1 = body.select("span.hhmm");

        System.out.println("조회 일자: "+times.text()+" ,조회 시간: "+times1.text());
        System.out.println("=====================================================================");

        MelonTime melonTime = new MelonTime(times.text(),times1.text());
        timeInfo.add(melonTime);
    }

    public List<MelonTime> getTimeInfo(){
        return timeInfo;
    }
    ...
    public void top100Info(){ // TOP100(제목+가수+앨범URL) 출력 코드
        ...
        for (int i = 0; i < 100; i++) {
                ...
                Music music = new Music(rank.text(),title.text(),singer.text(),albumJacketImage);
                top100.add(music);
